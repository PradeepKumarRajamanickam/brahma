...
impl Graph {
    ...
    pub fn add_dependency_package(&mut self, package: &str, version: &str) -> Result<(), String>
    {
        if let Some(v) = self.dep_package.get(package)
        {
            if v != version
            {
                return Err(format!("Version collision for package {}, current {} new {}",package,v,version));
            }
            else
            {
                return Ok(());
