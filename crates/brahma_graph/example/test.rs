...
fn main()
{
    let mut graph: Graph = Graph{ ..Default::default() };


    let state_id_start = get_start_state(&mut graph);
    let state_id_idle = get_idle_state(&mut graph);

    let transition_id_start_to_idle = get_start_to_idle_transition(
        &mut graph, 
        state_id_start,
        state_id_idle);

    // fsm
    graph.connect_fsm_control_ports(state_id_start, transition_id_start_to_idle)
    .unwrap();

    graph.connect_fsm_control_ports(transition_id_start_to_idle, state_id_idle)
    .unwrap();
...
fn get_start_state( graph: &mut Graph) -> u64
{
     ...
     let id_on_enter_event = graph.add_event_node(Event
        ...
        graph.connect_lane_control_ports(id_on_enter_event, id_print_method)
        .unwrap();
...
fn get_start_to_idle_transition(graph: &mut Graph, start: u64, end: u64) -> u64
{
        // node
        ...
        let id_transit = graph.add_transit_node(
            Transit{ owner: transition });
